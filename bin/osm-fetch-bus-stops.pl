...
if( ! Getopt::Long::GetOptions(
	...
	'bbox-bounded-by=s' => sub {
		$bbox = Geo::BoundingBox->new();
		if( ! $bbox->bounded_by($_[1]) ){ print STDERR "$0 : input bounded-by spec could not be parsed: '".$_[1]."'. Expected (lat:lon,lat:lon) for left-bottom and right-top corners of the bounding box.\n"; exit(1) }
		if( ! $engine->bbox($bbox) || ! $bbox->was_set() ){ print STDERR "$0 : failed to set the bounding box of the engine to $bbox\n"; exit(1) }
	},
...
if( ! defined $plug ){ print STDERR "$0 : call to ".'Geo::OSM::Overpass::Plugin::FetchBusStops->new()'." has failed.\n"; exit(1) }
